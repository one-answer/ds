...
# 交易参数配置 - 结合两个版本的优点
TRADE_CONFIG = {
    'symbol': 'TRX/USDT:USDT',  # OKX的合约符号格式
    'amount': 2,  # 交易数量 (TRX)
    'leverage': 5,  # 杠杆倍数
...
def analyze_with_deepseek(price_data):
    ...
    prompt = f"""
    你是一个专业的加密货币交易分析师。请基于以下TRX/USDT {TRADE_CONFIG['timeframe']}周期数据进行分析：
    ...
    - 本K线成交量: {price_data['volume']:.5f} TRX
...
def trading_bot():
    ...
    print(f"TRX当前价格: ${price_data['price']:,.5f}")
